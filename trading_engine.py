...
def fetch_whale_and_liquidations_simulation(symbol):
    """
    محاكاة رصد تصفية العقود وحركات الحيتان الكبرى (Whale & Liquidations Tracker)
    """
    try:
        # محاكاة بيانات حية مبنية على تقلبات الأصل
        rand_val = np.random.choice(['Bullish Accumulation', 'Bearish Dump Warning', 'Neutral Consolidation', 'Massive Whale Long Inflow'], p=[0.3, 0.2, 0.3, 0.2])
        liq_amount = round(np.random.uniform(5.5, 45.2), 2)
        side_liq = np.random.choice(['Shorts Liquidated', 'Longs Liquidated'])
        return {
            "whale_status": rand_val,
            "liquidation_alert": f"${liq_amount}M {side_liq}"
        }
    except Exception:
        return {"whale_status": "Normal Market Flow", "liquidation_alert": "$0.0M None"}
...
def calculate_indicators(df, ema_period=200, rsi_period=14, atr_period=10):
    ...
    if len(clean_ml) > 40:
        ...
        model = GradientBoostingClassifier(n_estimators=150, learning_rate=0.03, max_depth=4, random_state=42)
    ...
    conditions = [consensus_score > 55, consensus_score < 45]
    choices = ["BUY", "SELL"]
